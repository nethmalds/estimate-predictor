...
export default async function DashboardPage() {
  ...
  const stats = [
    {
      ...
      value: summary?.total_estimates ?? 0,
    ...
    {
      label: "This Month",
      value: summary?.estimates_this_month ?? 0,
    ...
    },
    {
      ...
      value: summary ? `${((summary.average_confidence ?? 0) * 100).toFixed(1)}%` : "—",
    ...
    },
    {
      ...
      value: summary ? `${(summary.total_estimated_value / 1_000_000).toFixed(1)}M` : "—",
  ...
  return (
    <div className="relative mx-auto max-w-6xl space-y-8 overflow-hidden">
      ...
      <div className="pointer-events-none absolute -top-24 right-1/4 -z-10 h-[300px] w-[500px] rounded-full bg-blue-600/10 blur-[120px]" />
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border/60 hover:bg-transparent">
                ...
                <TableHead className="hidden text-zinc-500 sm:table-cell">Date</TableHead>
                <TableHead className="hidden text-zinc-500 sm:table-cell">Confidence</TableHead>
                  ...
                  <TableCell>
                    <Link href={`/dashboard/estimates/${est.id}`} className="block">
                      ...
                      <p className="mt-0.5 text-xs text-zinc-500">
                        {est.building_type && (
                          <span className="capitalize">{est.building_type}</span>
                        )}
                        ...
                        {est.floorplan_accepted != null && (
                          <span
                            className={cn(
                              "ml-2",
                              est.floorplan_accepted ? "text-green-400" : "text-zinc-600"
                            )}
                          >
